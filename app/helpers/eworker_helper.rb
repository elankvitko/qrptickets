module EworkerHelper
  def compare_pb
    ...
    CSV.foreach( 'purchasing.csv', headers:true, :header_converters=> lambda {|f| f.strip}, :converters=> lambda {|f| f ? f.strip : nil} ) do | purchasing |
      if purchasing_ndc_duplicates[ purchasing[ "Universal Ndc #" ].gsub( '0', '' )  ]
        purchasing_ndc_duplicates[ purchasing[ "Universal Ndc #" ].gsub( '0', '' )  ][ 3 ] += purchasing[ "Quantity Billed" ].to_i
      else
        purchasing_ndc_duplicates[ purchasing[ "Universal Ndc #" ].gsub( '0', '' )  ] = [ purchasing[ "Item Description" ], purchasing[ "Item Type" ], purchasing[ "Size" ].to_i, purchasing[ "Quantity Billed" ].to_i ]
      end
    end
